...
public sealed class PresTrustUserContext : IPresTrustUserContext
{
    ...
    public void DeriveUserProfileFromUserId(string userid)
    {
        ...
        if (string.Compare(agencyUser, "farm-agencyadmin", true) == 0)
        {
        ...
        if (string.Compare(agencyUser, "farm-agencysignatory", true) == 0)
        {
        ...
        if (string.Compare(agencyUser, "farm-agencyreadonly", true) == 0)
        {
    ...
    private void DeriveUserProfile()
    {
        ...
        if (this.accessor.HttpContext.User.IsInRole(IdentityRoles.FARM_PROGRAM_ADMIN))
            userProfile.Role = UserRoleEnum.PROGRAM_ADMIN;

        if (this.accessor.HttpContext.User.IsInRole(IdentityRoles.FARM_PROGRAM_EDITOR))
            userProfile.Role = UserRoleEnum.PROGRAM_EDITOR;

        if (this.accessor.HttpContext.User.IsInRole(IdentityRoles.FARM_PROGRAM_COMMITTEE))
            userProfile.Role = UserRoleEnum.PROGRAM_COMMITTEE;

        if (this.accessor.HttpContext.User.IsInRole(IdentityRoles.FARM_PROGRAM_READONLY))
            userProfile.Role = UserRoleEnum.PROGRAM_READONLY;
